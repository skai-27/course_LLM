...
- 실행 파라미터 (PARAMETER)
  ...
  - `temperature 0`: 답변의 무작위성을 제거하여 항상 가장 확률이 높은 답변을 하도록 설정합니다. (일관된 답변이 필요한 경우 사용)
  - `num_predict 3000`: 모델이 한 번에 생성할 수 있는 최대 토큰 수를 3000개로 제한합니다.
  - `num_ctx 4096`: 모델이 기억할 수 있는 이전 대화 내용(컨텍스트 창)의 길이를 4096 토큰으로 설정합니다.
  - `stop <s> / stop </s>`: 모델이 답변을 생성하다가 이 토큰들을 만나면 생성을 중단하도록 설정하여, 불필요한 텍스트가 뒤에 붙는 것을 방지합니다.
